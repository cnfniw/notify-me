...
public class CommentStrategy implements NotifyStrategy {
    ...
    public void process(NotifyBaseEvent event, NotifyMe setting) {
        ...
        // 如果开启了 selfNotify，admin 自己的评论也推送
        boolean isAdmin = commentSpec.getOwner().getName().equals("admin");
        if (isAdmin && !setting.getSelfNotify()) {
            return;
        }
        if (commentInfo.getMetadata().getDeletionTimestamp() == null) {
            // 不是管理员的话就推送通知
            ...
            postInfo.ifPresent(post -> {
                if (setting.getSelfNotify() || !commentSpec.getOwner().getName().equals(post.getSpec().getOwner())) {
                    // 文章作者自己回复的不推送通知（除非开启了 selfNotify）
